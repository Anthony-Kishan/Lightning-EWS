...
export default function HistoryTable({ events, deviceName }) {
  const downloadCSV = () => {
    const headers = "Timestamp,Type,Level,Distance(km)\n";
    const rows = events.map(e => 
      `${new Date(e.ts).toISOString()},${e.type},${e.level},${e.distanceKm}`
    ).join("\n");
    const blob = new Blob([headers + rows], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `lightning_report_${deviceName || 'device'}.csv`;
    a.click();
  };
